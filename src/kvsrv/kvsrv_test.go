...
func TestReliablePut(t *testing.T) {
	...
	ck := ts.MakeClerk()
	if err := ck.Put("k", Val, Ver); err != rpcapi.OK {
		t.Fatalf("Put err %v", err)
	}

	if val, ver, err := ck.Get("k"); err != rpcapi.OK {
	...
	if err := ck.Put("k", Val, 0); err != rpcapi.ErrVersion {
		t.Fatalf("expected Put to fail with ErrVersion; got err=%v", err)
	}

	if err := ck.Put("y", Val, rpcapi.Tversion(1)); err != rpcapi.ErrNoKey {
		t.Fatalf("expected Put to fail with ErrNoKey; got err=%v", err)
	}

	if _, _, err := ck.Get("y"); err != rpcapi.ErrNoKey {
...
func TestMemPutManyClientsReliable(t *testing.T) {
	...
	for i := 0; i < NCLIENT; i++ {
		if err := cks[i].Put("k", "", 1); err != rpcapi.ErrNoKey {
	...
	for i := 0; i < NCLIENT; i++ {
		if err := cks[i].Put("k", v, rpcapi.Tversion(i)); err != rpcapi.OK {
			t.Fatalf("Put failed %v", err)
		}
	}
...
func TestUnreliableNet(t *testing.T) {
	...
	for try := 0; try < NTRY; try++ {
		for i := 0; true; i++ {
			if err := ts.PutJson(ck, "k", i, rpcapi.Tversion(try), 0); err != rpcapi.ErrMaybe {
				if i > 0 && err != rpcapi.ErrVersion {
		...
		v := 0
		if ver := ts.GetJson(ck, "k", 0, &v); ver != rpcapi.Tversion(try+1) {

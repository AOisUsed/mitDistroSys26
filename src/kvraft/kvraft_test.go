...
func (ts *Test) GenericTestSpeed() {
	...
	for i := 0; i < numOps; i++ {
		s := time.Now()
		if err := ck.Put("k", strconv.Itoa(i), rpcapi.Tversion(i)); err != rpcapi.OK {
	...
	if _, ver, err := ck.Get("k"); err != rpcapi.OK {
...
func TestOnePartition4B(t *testing.T) {
	...
	ver0 := ts.PutAtLeastOnce(ck, "1", "13", rpcapi.Tversion(0), -1)
	...
	done0 := make(chan rpcapi.Tversion)
	done1 := make(chan rpcapi.Tversion)
...
func TestSnapshotRPC4C(t *testing.T) {
	...
	vera := ts.PutAtLeastOnce(ck, "a", "A", rpcapi.Tversion(0), -1)
	ts.CheckGet(ck, "a", "A", vera)

	verb := rpcapi.Tversion(0)
	...
	tester.AnnotateTwoPartitions([]int{0, 1}, []int{2})
	{
		...
		for i := 0; i < 50; i++ {
			ts.PutAtLeastOnce(ck1, strconv.Itoa(i), strconv.Itoa(i), rpcapi.Tversion(0), -1)
		}
		time.Sleep(kvtest.ElectionTimeout)
		verb = ts.PutAtLeastOnce(ck1, "b", "B", rpcapi.Tversion(0), -1)
	}
	...
	verc := rpcapi.Tversion(0)
	ts.Group(Gid).Partition([]int{0, 2}, []int{1})
	tester.AnnotateTwoPartitions([]int{0, 2}, []int{1})
	{
		ck1 := ts.MakeClerkTo([]int{0, 2})
		verc = ts.PutAtLeastOnce(ck1, "c", "C", rpcapi.Tversion(0), -1)
		ts.PutAtLeastOnce(ck1, "d", "D", rpcapi.Tversion(0), -1)
		ts.CheckGet(ck1, "a", "A", vera)
		ts.CheckGet(ck1, "b", "B", verb)
		ts.CheckGet(ck1, "1", "1", rpcapi.Tversion(1))
		ts.CheckGet(ck1, "49", "49", rpcapi.Tversion(1))
	}
	...
	vere := ts.PutAtLeastOnce(ck, "e", "E", rpcapi.Tversion(0), -1)
	ts.CheckGet(ck, "c", "C", verc)
	ts.CheckGet(ck, "e", "E", vere)
	ts.CheckGet(ck, "1", "1", rpcapi.Tversion(1))
}

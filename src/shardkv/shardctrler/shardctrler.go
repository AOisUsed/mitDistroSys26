...
func (sck *ShardCtrler) InitController() {
	...
	if currentConfig.Num < nextConfig.Num {
		debug.ObserveMigrationFaultPrintf("分片控制器: 发现有未完成的分片迁移，恢复中")
...
func (sck *ShardCtrler) migrateShards(oldCfg *shardcfg.ShardConfig, ver rpcapi.Tversion, newCfg *shardcfg.ShardConfig, fromRecovery bool) bool {
		...
		go func(shid shardcfg.Tshid, oldGid tester.Tgid, newGid tester.Tgid) {
			...
			for err == rpcapi.ErrRetryExhausted {
				...
				if err == rpcapi.ErrRetryExhausted {
					debug.ObserveMigrationFaultPrintf("分片控制器: 冻结分片(%v), Config #%v -> 组 %v, 错误码：%v", shid, newCfg.Num, newGid, err)
			...
			for err == rpcapi.ErrRetryExhausted {
				...
				if err == rpcapi.ErrRetryExhausted {
					debug.ObserveMigrationFaultPrintf("分片控制器: 安装分片(%v), Config #%v -> 组 %v, 错误码: %v", shid, newCfg.Num, newGid, err)
					if sck.isSuperseded(newCfg) {
						isSuperseded.Store(true)
						return
					}
			...
			for err == rpcapi.ErrRetryExhausted {
				...
				if err == rpcapi.ErrRetryExhausted {
					debug.ObserveMigrationFaultPrintf("分片控制器: 删除分片(%v), Config #%v -> 组 %v, 错误码：%v", shid, newCfg.Num, newGid, err)
					if sck.isSuperseded(newCfg) {
						isSuperseded.Store(true)
						return
					}
				...
				if attempts >= maxAttempts && fromRecovery { // if this is from recovery, only try maxAttempts times. and if all fails, we can (almost safely) conclude that the group left
					debug.ObserveMigrationFaultPrintf("分片控制器: 删除分片(%v), Config #%v -> 组 %v 重试第 %v 次仍无响应, 组应已离开，放弃操作", shid, newCfg.Num, newGid, attempts)
					break
				}
			}
	...
	if isSuperseded.Load() { // if shard migration fails at any stage, don't save newCfg to configStore !
		debug.ObserveMigrationFaultPrintf("分片控制器：分片迁移中发现配置号 %v 已过期，放弃此次迁移", newCfg.Num)
		return false
	}

	...
	for shid, oldGid := range oldCfg.Shards {
		...
		go func(shid shardcfg.Tshid, oldGid tester.Tgid, newGid tester.Tgid) {
			...
			for err == rpcapi.ErrRetryExhausted {
				...
				debug.ObserveMigrationPrintf("controller: FreezeShard(shard = %v, num = %v)-> group %v, err=%v", shid, newCfg.Num, newGid, err)
			...
			for err == rpcapi.ErrRetryExhausted {
				...
				debug.ObserveMigrationPrintf("controller: InstallShard(shard = %v, num = %v)-> group %v, err=%v", shid, newCfg.Num, newGid, err)
				if err == rpcapi.ErrRetryExhausted {
					if sck.isSuperseded(newCfg) {
						isSuperseded.Store(true)
						return
			...
			for err == rpcapi.ErrRetryExhausted {
				...
				debug.ObserveMigrationPrintf("controller: DeleteShard(shard = %v, num = %v)-> group %v, err=%v", shid, newCfg.Num, newGid, err)
				if err == rpcapi.ErrRetryExhausted {
					if sck.isSuperseded(newCfg) {
						isSuperseded.Store(true)
						return

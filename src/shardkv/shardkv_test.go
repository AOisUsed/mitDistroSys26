...
func TestStaticOneShardGroup5A(t *testing.T) {
	...
	n := len(ka)
	for i := 0; i < n; i++ {
		ts.CheckGet(ck, ka[i], va[i], rpcapi.Tversion(1)) // check the puts
	}
...
func TestPartitionControllerJoin5C(t *testing.T) {
	...
	go func() {
		...
		ts.Config.MakeGroupStart("shardgrp", args, ngid, NSRV)

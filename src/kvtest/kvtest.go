...
type IKVClerk interface {
	Get(string) (string, rpcapi.Tversion, rpcapi.Err)
	Put(string, string, rpcapi.Tversion) rpcapi.Err
}
...
func (tck *TestClerk) Put(key string, value string, version rpcapi.Tversion) rpcapi.Err {
	tck.Cfg.OpInc()
	return tck.IKVClerk.Put(key, value, version)
}

func (tck *TestClerk) Get(key string) (string, rpcapi.Tversion, rpcapi.Err) {
	tck.Cfg.OpInc()
...
func (ts *Test) PutAtLeastOnce(ck IKVClerk, key, value string, ver rpcapi.Tversion, me int) rpcapi.Tversion {
	verPrev := ver
	for true {
		...
		if err == rpcapi.ErrMaybe || err == rpcapi.ErrVersion {
			ver += 1
		} else {
...
func (ts *Test) CheckGet(ck IKVClerk, key, value string, version rpcapi.Tversion) {
...
func (ts *Test) CheckPutConcurrent(ck IKVClerk, key string, rs []ClntRes, res *ClntRes, reliable bool) {
	...
	if reliable {
		if ver0 != rpcapi.Tversion(res.Nok) {
			ts.Fatalf("Reliable: Wrong number of puts: server %d clnts %v", ver0, res)
		}
	} else if ver0 > rpcapi.Tversion(res.Nok+res.Nmaybe) {
...
func (ts *Test) GetJson(ck IKVClerk, key string, me int, v any) rpcapi.Tversion {
	if val, ver, err := Get(ts.Config, ck, key, ts.oplog, me); err == rpcapi.OK {
...
func (ts *Test) PutJson(ck IKVClerk, key string, v any, ver rpcapi.Tversion, me int) rpcapi.Err {
...
func (ts *Test) PutAtLeastOnceJson(ck IKVClerk, key string, value any, ver rpcapi.Tversion, me int) rpcapi.Tversion {
	for true {
		if err := ts.PutJson(ck, key, value, 0, me); err != rpcapi.ErrMaybe {
			break
		}
		ver += 1
	}
...
func (ts *Test) OnePut(me int, ck IKVClerk, key string, ver rpcapi.Tversion) (rpcapi.Tversion, bool) {
	for true {
		err := ts.PutJson(ck, key, EntryV{me, ver}, ver, me)
		if !(err == rpcapi.OK || err == rpcapi.ErrVersion || err == rpcapi.ErrMaybe) {
			ts.Fatalf("Wrong error %v", err)
		}
		e := EntryV{}
		ver0 := ts.GetJson(ck, key, me, &e)
		if err == rpcapi.OK && ver0 == ver+1 { // my put?
			if e.Id != me && e.V != ver {
				ts.Fatalf("Wrong value %v", e)
			}
		}
		ver = ver0
		if err == rpcapi.OK || err == rpcapi.ErrMaybe {
			return ver, err == rpcapi.OK
		}
	}
	return 0, false
}
...
func (ts *Test) OneClientPut(cli int, ck IKVClerk, ka []string, done chan struct{}) ClntRes {
	res := ClntRes{}
	verm := make(map[string]rpcapi.Tversion)
	for _, k := range ka {
		verm[k] = rpcapi.Tversion(0)
	}
	ok := false
	for true {
		select {
...
func (ts *Test) SpreadPutsSize(ck IKVClerk, n, valsz int) ([]string, []string) {
	...
	for i := 0; i < n; i++ {
		...
		ck.Put(ka[i], va[i], rpcapi.Tversion(0))
	}
	for i := 0; i < n; i++ {
		ts.CheckGet(ck, ka[i], va[i], rpcapi.Tversion(1))
	}
	return ka, va
}
	...
	for i := 0; true; i++ {
		...
		default:
			...
			for true {
				...
				if err := ts.PutJson(ck, "k", es, ver, me); err == rpcapi.OK {
					nok += 1
					break
				} else if err == rpcapi.ErrMaybe {
...
// correct tuples.
func (ts *Test) CheckAppends(es []EntryN, nclnt int, rs []ClntRes, ver rpcapi.Tversion) {

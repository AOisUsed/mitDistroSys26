...
func Get(cfg *tester.Config, ck IKVClerk, key string, log *OpLog, cli int) (string, rpcapi.Tversion, rpcapi.Err) {
...
func Put(cfg *tester.Config, ck IKVClerk, key string, value string, version rpcapi.Tversion, log *OpLog, cli int) rpcapi.Err {
	start := int64(time.Since(t0))
...
// Porcupine
func (ts *Test) Get(ck IKVClerk, key string, cli int) (string, rpcapi.Tversion, rpcapi.Err) {
	start := int64(time.Since(t0))
	val, ver, err := ck.Get(key)
	end := int64(time.Since(t0))
	ts.OpInc()
...
// Porcupine
func (ts *Test) Put(ck IKVClerk, key string, value string, version rpcapi.Tversion, cli int) rpcapi.Err {
	start := int64(time.Since(t0))
	err := ck.Put(key, value, version)
	end := int64(time.Since(t0))
	if ts.oplog != nil {

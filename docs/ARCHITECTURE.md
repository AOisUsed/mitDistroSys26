...
- 外层 **ShardKV Clerk**: 面向用户的统一入口。收到 Get/Put 请求后，计算分片、查询配置缓存、确定目标分片组。
- 内层 **ShardGroup Clerk**: 面向分片组的通信代理。在组内轮询找到 Leader 节点，发送请求。
...
ShardKV Clerk 持有分片控制器的引用用于查询配置。客户端通过两级路由让服务端的分片以及分片组多节点对用户透明，使用体验如同操作一个单机的系统。
...
分片控制器负责管理分片配置和编排分片迁移。它通过配置仓库的 [CAS](https://zh.wikipedia.org/wiki/比较并交换) 写入协调配置变更，调用分片组的冻结/安装/删除接口迁移数据。ShardKV Clerk 持有它的引用用于查询配置。
...
| 功能    | 说明                                  |
|-------|-------------------------------------|
| 接受请求  | 接受用户键值读写请求并交给 ShardGroup Clerk      |
| 分片计算  | 对 key 做哈希计算所属分片                     |
| 配置缓存  | 本地缓存分片配置，按需刷新                       |
| 查询配置  | 采用单飞模式 (single-flight) ，多请求并发时合并为一次 |
...
| 功能     | 说明                                                       |
|--------|----------------------------------------------------------|
| 版本化读写  | 每个键维护版本号，[CAS](https://zh.wikipedia.org/wiki/比较并交换) 写入   |
...
**职责：** 管理分片配置，编排分片迁移。与客户端结构类似，使用两级路由。
...
### 分片迁移流程
当分片组加入或离开集群时，分片需要重新分布，以下为迁移流程

> 注：从可读性考虑，仅包含正常执行流程，异常情况处理和幂等设计详见 [分片迁移](DESIGN.md)

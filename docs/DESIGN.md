...
此外，日志无限制增长会造成存储压力，因此一般需要[**日志压缩**](#日志压缩)机制: 记录截止到某条日志时状态机的状态，并删除其此前的日志以释放存储。
...
领导者在需要给追随者发送日志时可能由于日志压缩，失去了这些日志，因此转为以 [`InstallSnapshot RPC`](API.md#installsnapshot)发送快照。 

...
> 📖 **阅读建议：** 推荐按 "先建立直觉、再查细节" 的方式阅读。
> - 先读「Raft 速览」建立整体印象；
> - 「核心数据结构」和各种表格不必详读，作为参考后续查阅即可；
> - 机制小节（选举 / 复制 / 快照 / 共识 / 应用）等建议顺序读，可跳过不关心的细节。

### Raft 速览

> 💡 **提示：** 此小节是对 Raft 的最简介绍，理论和细节可参考 [Raft 论文中文版](RAFT_PAPER_ZH.md)，或访问 [Raft 官网](https://raft.github.io)（包含一个可交互演示）

Raft 解决一个问题：在分布式系统中，如何在保证强一致性的前提下，实现服务的高可用性。做法是设立冗余节点副本，这些节点行为完全一致，对外表现像同一台单机，这样一些节点宕机/网络分区时，其他节点仍可提供数据一致的服务。因此，问题转化为：多个节点如何对命令的操作顺序达成一致，即使面临网络延迟、分区和节点故障。

核心思路：**赋予服务器节点角色和相应的行为模式 —— 在所有节点选出一个领导者，所有客户端请求都经由领导者转送给其他节点 (追随者) 确认；待大多数节点都确认后，领导者就执行请求，并通知其他节点也执行请求；最后领导者将请求结果返回给客户端。** 
...
- **命令共识**（保证全节点操作一致）：
  ...
  2. [**共识推进**](#共识推进)：领导者依据多数派确认，推进共识进度并同步给追随者。
  3. [**状态应用**](#状态应用)：所有节点将已提交日志顺序应用到状态机。
...
| `LastIncludedTerm`  | `int`        | `LastIncludedIndex`处日志的`Term` (注：本实现中，日志数组首位保留 dummy log entry 作为`LastIncludedIndex`处的日志。该信息实际保存在此日志条目中） |
...
| 命令共识  | applier               | 1       | 将已共识日志 / 快照发送给上层 RSM, 驱动状态机应用到状态  | 收到`applyReadyCh` 信号      |
...
- 其余协程则按照「日志复制→日志提交→日志应用」的流程协同实现 **命令共识**：
  1. 领导者节点收到客户端请求后包装为日志，追加到 Raft 本地，随后唤醒`replicateDispatcher` 分发日志给追随者
  2. `replicateDispatcher` 被唤醒后，找到节点日志落后于自身的追随者，并发唤醒对应的 `replicateWorker` 以 RPC 形式发送日志/快照给落后节点
  3. RPC 返回时，检查日志是否获得多数派确认。是则说明日志条目获得了集群共识，通知 `applier` 将日志中的命令投递给状态机
...
在此模型之上，协程间信号传递采用了一套统一的通信模式，以实现异步推进共识进度，下文展开说明。

#### 协程间异步通信

常驻协程间通过 channel 异步通信，表达 「有任务待完成」 语义：
...
以上搭建了共识机制的并发骨架：5 类常驻协程通过 channel 信号协同推进「日志复制 → 日志提交 → 日志应用」的共识流程。下文将逐一展开各机制的实现细节，每一节对应上述某类协程的具体行为：
...
- [**日志压缩与快照安装**](#日志压缩与快照安装) ：如何使用快照来压缩日志以节省存储，加快日志重放。
- [**持久化与故障恢复**](#持久化与故障恢复)：节点重启后的状态重建。
...
> 
> 反之，如果**网络状况较差，机器宕机不是故障原因**，则应该设置较长的超时时间以避免网络震荡产生的领导权频繁变更，进而导致无法提供服务——代价是故障恢复更慢。

#### 发起选举

当 `ticker` 检测到超时后，调用 `startElection()` 发起选举。首先增加任期，转为候选人，为自己投票，并更新 `lastHeardTime` 以防止选举触发器立即再次触发选举。接下来给其他节点发送 [RequestVote RPC](API.md#requestvote) 获取选票；获得超过半数节点的投票则当选为领导者，并初始化每个追随者的 `nextIndex` 和 `matchIndex`.
...
`Start(command)` 是 RSM 层向 Raft 层发送共识请求的入口。领导者先把命令追加到自己的日志并持久化，再通知分发协程，然后立即返回——复制与提交在后台异步完成。RSM 对非领导者节点的请求会被拒绝，因为只有领导者可发起共识。
...
这一阶段把「有日志追加」这一信号，异步地投递到每个追随者的专属工作协程：`replicationDispatcher` 判定日志落后节点，并唤醒对应 `replicationWorker`.
...
`replicationWorker` 被唤醒后调用 `synchroniseFollower()` 对追随者循环发送 RPC 直到目标节点日志追平或此节点失去领导权。

其中，`replicateToFollower(i)` 会根据情况在 `AppendEntries` 与 `InstallSnapshot` 两种 RPC 间择一发送，详见 [批量复制](#批量复制) 与 [快照安装](#快照安装)。

> 🌟 **设计决策**：`synchroniseFollower()` 在 RPC 调用失败后不立即重试，而采用指数退避（以 100ms 为基准、2s 为上限，RPC 成功则回正退避时间），可以防止因目标节点宕机而大量重试，造成网络拥塞。

#### 批量复制

领导者通过 [AppendEntries RPC](API.md#appendentries) 来进行批量日志复制。

领导者为每个追随者记录两个字段：`nextIndex`（下一次要发送的日志起点）与 `matchIndex`（已确认匹配的日志终点）。每次发送 `AppendEntries` 都以 `nextIndex` 为起始位置；复制成功后，`matchIndex` **单调递增地**推进到本次发送日志的终点。 始终满足 `nextIndex > matchIndex`. 

追随者则根据 RPC 中字段来判断：若本地 `PrevLogIndex` 处日志存在且 `Term` 与 `PrevLogTerm` 匹配，便接受复制。因为 Raft 的 [**日志匹配性质**](RAFT_PAPER_ZH.md#5-raft-共识算法) 保证这一点：如果追随者与领导者在同一位置日志条目的 `Term` 匹配，则此日志及此前的所有日志都匹配。

![接受日志复制](images/appendEntries_accept.svg)

上图按时间顺序展示了一次成功的日志复制：
1. **发送**：领导者发送 `AppendEntries` (`PrevLogIndex=12`, `PrevLogTerm=6`).
2. **回复**：追随者发现 `Index=12`处日志 `Term` 与 `PrevLogTerm` 吻合，接受此 RPC，随后覆盖 13-14 号日志，追加 15 号日志。最后返回 `Success=true`
3. **更新**：领导者收到 `Success`, 更新此追随者的 `matchIndex=15`, `nextIndex=16`.
...
修复冲突的方式是：找到追随者与领导者**最后一条匹配日志**的位置，把追随者该位置之后的日志整体覆盖为领导者的。关键是「如何找到**最后一条匹配日志**的位置」。最朴素的做法是，每次 RPC 回退一条日志，直到发现匹配点。本实现则使用优化：通过让追随者在 RPC 回复中携带额外信息(`ConflictTerm` / `ConflictIndex`), 使领导者能一次最多回退整个任期内所有日志，以加快消除冲突。
...
1. **发送**：领导者发出 `AppendEntries`（`PrevLogIndex=12, PrevLogTerm=6`）。
2. **回复**：追随者在 12 号处的日志任期为 5，与声明不符，拒绝。随后找到 12 号日志所属任期（`Term=5`）的**第一条**索引，将其信息 `ConflictTerm=5, ConflictIndex=9` 连同 `Success=false` 返回——语义是 "冲突日志属于任期 5，请在该任期内找到匹配日志"。
...
1. **发送**：领导者发出 `AppendEntries`（`PrevLogIndex=12, PrevLogTerm=6`）。
2. **回复**：追随者发现自己日志过短，未达到 `PrevLogIndex=12`，拒绝。然后将 `ConflictTerm=-1, ConflictIndex=10`（`10` 即 `LastLogIndex+1`）连同 `Success=false` 返回——语义是 "该位置无日志，请从我最后一条日志（9 号）之后开始发送"。
...
1. **发送**：领导者发出 `AppendEntries`（`PrevLogIndex=12, PrevLogTerm=6`）。
2. **回复**：追随者中 `PrevLogIndex=12` 已被压缩，无法判断此处 `Term` 是否匹配，拒绝。之后将 `ConflictTerm=-1, ConflictIndex=15`（`15` 即 `LastIncludedIndex+1`）连同 `Success=false` 返回——语义是 "该位置日志被压缩无法判断，请从我的快照之后开始发送"。
3. **推后**：领导者据此回复将 `nextIndex` 推后到 15。

冲突 2、3 实质都是领导者声明的 `PrevLogIndex` 处日志在追随者中不存在。追随者需要通过合理的回复引导领导者将这两种冲突转化为冲突 1 类型。
...
另有一种特殊情况：由于日志压缩，冲突可能无法使用 `AppendEntries` 解决，如下图所示:

![日志缺失转快照](images/appendEntries_entries_short.svg)

追随者本地已无日志 10-11，领导者只能转为使用 `InstallSnapshot` 发送快照。具体细节见下一节中 [快照安装](#快照安装) 。
...
首先，RSM 将应用层的状态机记录为快照，再把快照内容 `snapshot` 和此快照中包含的最大日志索引`index` 作为结果返回给 Raft. Raft 则依此持久化快照数据，将日志数组截断至 `index` 处（保留此处日志作为 dummy entry），再更新 `index` 作为新的 `LastIncludedIndex`.

具体触发时机和条件见 [RSM](#rsm) 小节。

#### 快照安装

日志压缩产生一个问题：领导者在需要给追随者发送日志时可能失去了这些日志——因此转为以 [`InstallSnapshot RPC`](API.md#installsnapshot)发送快照（注：本实现的快照发送不使用[论文](RAFT_PAPER_ZH.md#7-日志压缩)中的分块，而是单次发送全量，流程有所简化），如下图所示：

![安装快照](images/installSnapshot.svg)

1. **发送**：领导者发现此追随者的 `nextIndex` 为 10，比本地最小的 12 号日志还低，转为发送覆盖了 1-11 号日志的快照。
2. **回复**：追随者发现本地快照进度较后(或无快照)，于是接受 `InstallSnapshot` 并保存其快照数据 `Data`。
3. **更新**：领导者收到成功回复（自身 `Term` 与回复中的相等），**单调递增地**更新此追随者的 `matchIndex` 到 11，`nextIndex` 到 12。

与「日志复制」相比，「快照安装」不会产生冲突，因为快照是已获共识命令在状态机上执行的结果；无论领导者变更，发送的快照始终是已获得共识的。只需注意一点：因网络延迟而收到的旧快照不应覆盖本地的新快照，否则会产生日志空洞。

接受快照后，如果快照状态比当前状态机新，则需要用快照重置状态机，即「快照应用」，详见下一节 [状态应用](#状态应用)。

### 共识推进与状态应用

#### 共识推进

当一条日志被复制到集群半数以上节点后，即获得了集群共识，Raft 以 `commitIndex` 来标记已知最大共识进度。

 `commitIndex` 的推进基于领导者与追随者之间的 RPC ，但不同角色的推进方式和时机各异，如下文所示。

**领导者**：主动推进 `commitIndex`, 并通知追随者共识进度

![领导者共识推进](images/advance_commit_leader.svg)

领导者为每个追随者记录 `matchIndex`，追踪对应日志复制进度。每次 `AppendEntries` / `InstallSnapshot`成功，且使对应节点的 `matchIndex` 增加后，就调用 `advanceCommitIndex()` 尝试推进 `commitIndex`：

从后向前扫描日志，找到满足以下三个条件的最大索引 `N`：
  1. `N` > 当前 `commitIndex`
  2. `Log[N].Term == CurrentTerm`（仅提交当前任期的日志，原因见 [论文图 8](RAFT_PAPER_ZH.md#542-提交先前任期的日志条目)）
  3. 超过半数节点上 `matchIndex >= N`

若找到，则更新 `commitIndex = N`. 

**追随者**：被动地通过 RPC 从领导者处获知共识进度

![追随者共识推进](images/advance_commit_follower.svg)

收到有效的 `AppendEntries` 后读取其中字段 `LeaderCommit`—— 若大于自身的 `commitIndex`, 则尽可能推进 `commitIndex`.

同样，收到有效的 `InstallSnapshot` 后，检查快照的 `LastIncludedIndex`, 如果大于自身的 `commitIndex`, 则更新。

#### 状态应用

在共识进度推进后，节点即可把新获得共识的内容作用于状态机上。

这项工作由 `applier` 协程串行完成。由于共识推进的来源多样且并发，使用单例 `applier` 协程串行修改状态机可以避免无序并发操作破坏线性一致性。

应用流程如下图所示：

![状态应用流程图](images/apply_flow_chart.svg)

共识推进时，`applier` 被唤醒。唤醒后 `applier` 进入循环，不断将已共识，但未应用的内容包装为 [`ApplyMsg`](API.md#rsm-applymsg-结构) 通过 `applyCh` 发送给上层 RSM，直到 `lastApplied`(应用进度) 追上 `commitIndex`(共识进度)。此外，因为快照会截断日志，使`commitIndex` 跳跃式地推进，必须**优先应用快照**以跳过被截断的日志。

### 持久化与故障恢复

需要持久化的内容分为两类：一是安全关键数据（`CurrentTerm`、`VotedFor`、`Log`），二是快照（`LastIncludedIndex`、`SnapshotData`）。在更改时，这些数据必须立刻持久化，以保证 [Raft 安全性](RAFT_PAPER_ZH.md#5-raft-共识算法)。

节点重启时调用 `readPersist()` 从持久器中读取这些状态并恢复，使节点能够继续参与集群。
